...
import (
	...
	couchbasev1 "couchbase/api/couchbase-cluster/v1"
	"log"
	"net/http"
	"path/filepath"
)
...
var (
	deploymentResource = metav1.GroupVersionResource{Version: "v1", Resource: "couchbaseclusters"}
)
...
func applySecurityDefaults(req *v1beta1.AdmissionRequest) ([]patchOperation, error) {
	...
	raw := req.Object.Raw
	cbc := couchbasev1.CouchbaseCluster{}
	if _, _, err := universalDeserializer.Decode(raw, nil, &cbc); err != nil {
		return nil, fmt.Errorf("could not deserialize cbc object: %v", err)
	}

	// Retrieve the `runAsNonRoot` and `runAsUser` values.
	var deployMe *bool
	
	if cbc.Spec.DeployMe != nil {
		deployMe = pod.Spec.DeployMe
